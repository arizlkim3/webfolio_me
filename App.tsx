...
const App: React.FC = () => {
  ...
  // Mendapatkan nomor WA dari profile untuk header link
  ...
  return (
    <div className="min-h-screen flex flex-col bg-slate-50 dark:bg-slate-900 text-slate-900 dark:text-slate-200 font-sans transition-colors duration-300 relative overflow-x-hidden">
      ...
      {/* Navbar - Fixed Position so it never disappears */}
      <header className="fixed top-0 left-0 w-full z-[60] bg-white/20 dark:bg-slate-900/40 backdrop-blur-2xl border-b border-white/20 dark:border-slate-800/50 transition-colors duration-300 shadow-sm">
          ...
          <div className="flex justify-between items-center h-16">
            ...
            <div className="flex items-center gap-3 shrink-0">
              ...
              <div className="flex flex-col">
                <h1 className="text-lg md:text-xl font-bold bg-clip-text text-transparent bg-gradient-to-r from-slate-900 to-slate-600 dark:from-white dark:to-slate-400 leading-none uppercase tracking-tight">
                  4RISE
                </h1>
                <span className="text-[9px] text-slate-500 dark:text-slate-400 font-bold uppercase tracking-wider mt-0.5">
                  {role === 'creator' ? 'ADMIN' : 'PENGUNJUNG'}
                </span>
              </div>
            </div>
            ...
            <nav className="flex items-center gap-2 md:gap-4">
              ...
              <div className="hidden md:flex p-1 rounded-xl bg-slate-500/5 dark:bg-slate-400/5 border border-white/20 dark:border-slate-700/50 backdrop-blur-md">
                <button
                  ...
                  className={`px-3 py-1.5 md:px-5 md:py-2 rounded-lg text-xs md:text-sm font-black uppercase tracking-tight transition-all duration-200 ${
                    activeTab === 'view'
              ...
              <div className="flex items-center gap-1 md:gap-2">
                <a
                  ...
                  className="p-2 rounded-xl text-slate-500 dark:text-slate-400 hover:bg-green-500/10 hover:text-[#25D366] transition-all border border-transparent hover:border-green-500/20 active:scale-90"
                  title="Chat WhatsApp"
                >
                ...
                <button
                  onClick={toggleTheme}
                  className="p-2 rounded-xl text-slate-500 dark:text-slate-400 hover:bg-slate-500/10 dark:hover:bg-slate-400/10 transition-all border border-transparent hover:border-slate-200 dark:hover:border-slate-700 active:scale-90"
                  title="Ganti Tema"
                >
                  {theme === 'dark' ? (
                ...
                <button
                  ...
                  className="text-slate-400 dark:text-slate-500 hover:text-red-500 dark:hover:text-red-400 p-2 rounded-lg hover:bg-red-500/10 transition-all active:scale-90"
                  title="Keluar"
                >
                  <svg xmlns="http://www.w3.org/2000/svg" className="h-5 w-5" fill="none" viewBox="0 0 24 24" stroke="currentColor">
      ...
      {/* Konten Utama dengan Padding Top untuk mengimbangi Navbar Fixed */}
      <main className="flex-grow max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 pt-24 pb-24 md:pb-8 relative z-10 w-full">
        ...
        <>
          ...
          {isMobileMenuOpen && (
            ...
            >
              <button 
                onClick={() => setActiveTab('create')}
                className={`flex items-center gap-3 px-5 py-3 rounded-xl shadow-lg border border-white/20 dark:border-slate-700 backdrop-blur-xl transition-all ${activeTab === 'create' ? 'bg-secondary text-white' : 'bg-white/95 dark:bg-slate-800/95 text-slate-700 dark:text-slate-200'}`}
              >
              ...
              <button 
                onClick={() => setActiveTab('profile')}
                className={`flex items-center gap-3 px-5 py-3 rounded-xl shadow-lg border border-white/20 dark:border-slate-700 backdrop-blur-xl transition-all ${activeTab === 'profile' ? 'bg-secondary text-white' : 'bg-white/95 dark:bg-slate-800/95 text-slate-700 dark:text-slate-200'}`}
              >
          ...
          {/* Main Taskbar */}
          <nav className="md:hidden fixed bottom-6 left-1/2 transform -translate-x-1/2 w-[90%] max-sm max-w-sm bg-white/40 dark:bg-slate-900/40 backdrop-blur-xl border border-white/20 dark:border-slate-700 rounded-2xl shadow-2xl z-50 flex justify-between px-2 py-2">
            ...
            <div className="w-px h-8 bg-slate-500/10 dark:bg-slate-400/10 self-center mx-1"></div>

            <button
              onClick={() => setIsMobileMenuOpen(!isMobileMenuOpen)}
              className={`flex-1 flex flex-col items-center justify-center py-2 rounded-xl transition-all ${
                isMobileMenuOpen || activeTab === 'create' || activeTab === 'profile'
                  ? 'bg-secondary/20 text-secondary ring-1 ring-secondary/20'
                  : 'text-slate-400 hover:text-slate-600 dark:hover:text-slate-300'
              }`}
            >
      ...
      <footer className="border-t border-slate-200 dark:border-slate-800 bg-white/10 dark:bg-slate-950/10 backdrop-blur-md py-6 md:py-8 mt-auto transition-colors duration-300 pb-24 md:pb-8 relative z-10">
        <div className="max-w-7xl mx-auto px-4 text-center">
          <p className="text-slate-500 dark:text-slate-500 text-[10px] font-bold uppercase tracking-[0.3em]">
            &copy; {new Date().getFullYear()} 4RISE. Built with passion.
          </p>
        </div>
      </footer>
