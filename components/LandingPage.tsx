...
const LandingPage: React.FC<LandingPageProps> = ({ onLogin, theme, toggleTheme }) => {
  ...
  return (
    <div className="min-h-screen bg-slate-50 dark:bg-slate-900 flex flex-col items-center justify-center p-4 relative overflow-hidden transition-colors duration-500">
      ...
      {/* Floating Header Card for Landing Page - Slide down animation */}
      ...
      {/* Background decoration - Fluid Animation */}
      ...
      <div className="relative z-10 max-w-4xl w-full text-center space-y-12 px-4">
        <div className="space-y-6 flex flex-col items-center">
          <div className="inline-flex flex-col items-center group">
            ...
            {/* Subtitle - Sliding Up with delay */}
            ...
            {/* Social Media Links Container - Staggered reveal from left */}
        ...
        {/* Action Buttons - Scale up with bounce effect */}
      ...
      {/* RAMPING CRYSTAL BOKEH MODAL - max-w-[360px] */}
      {showLoginModal && (
        <div className="fixed inset-0 z-[100] flex items-center justify-center p-6 bg-slate-400/5 dark:bg-slate-950/5 backdrop-blur-[32px] animate-fade-in" onClick={() => setShowLoginModal(false)}>
           <div 
             className="w-full max-w-[360px] bg-white/60 dark:bg-slate-900/60 backdrop-blur-[120px] rounded-[3rem] border border-white/60 dark:border-white/10 shadow-[0_40px_100px_rgba(0,0,0,0.2)] dark:shadow-[0_60px_120px_rgba(0,0,0,0.5)] p-10 relative animate-scale-up" 
             onClick={(e) => e.stopPropagation()}
           >
              {/* Close Button - Integrated */}
              <button 
                onClick={() => {setShowLoginModal(false); setPassword(''); setError('');}} 
                className="absolute top-8 right-8 text-slate-500 dark:text-slate-400 hover:text-slate-950 dark:hover:text-white transition-all p-2 bg-black/5 dark:bg-white/5 rounded-full"
              >
                <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={3} d="M6 18L18 6M6 6l12 12" /></svg>
              </button>

              <div className="flex flex-col items-center mb-10">
                 <div className="w-14 h-14 rounded-2xl bg-secondary/30 dark:bg-secondary/40 flex items-center justify-center mb-5 ring-2 ring-white/70 dark:ring-white/10 shadow-lg">
                    <svg className="w-7 h-7 text-secondary dark:text-purple-200" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2.5} d="M12 15v2m-6 4h12a2 2 0 002-2v-6a2 2 0 00-2-2H6a2 2 0 00-2 2v6a2 2 0 002 2zm10-10V7a4 4 0 00-8 0v4h8z" /></svg>
                 </div>
                 <h2 className="text-xl font-black text-slate-950 dark:text-white uppercase tracking-tighter leading-none mb-2.5">Akses Pemilik</h2>
                 <span className="text-[9px] font-black uppercase tracking-[0.2em] text-primary dark:text-indigo-200 opacity-70">DASHBOARD ADMIN</span>
              </div>

              <form onSubmit={handleCreatorLogin} className="space-y-4">
                <div className="space-y-3">
                  <label className="block text-[10px] font-black text-slate-700 dark:text-slate-200 uppercase tracking-widest ml-1 opacity-80">KATA SANDI</label>
                  
                  {/* Inline Form with Improved Symmetry Alignment */}
                  <div className="flex items-center gap-2.5">
                    <input
                      ...
                      placeholder="Admin..."
                      className="flex-1 h-14 bg-white/40 dark:bg-black/30 border-2 border-white/80 dark:border-white/10 rounded-2xl px-6 text-slate-950 dark:text-white text-sm font-bold focus:ring-4 focus:ring-primary/40 focus:border-primary outline-none transition-all placeholder:text-slate-400 dark:placeholder:text-slate-600 shadow-xl"
                      autoFocus
                    />
                    <button 
                      type="submit" 
                      className="flex-shrink-0 w-14 h-14 aspect-square bg-gradient-to-br from-primary to-indigo-600 text-white rounded-2xl flex items-center justify-center shadow-lg shadow-primary/40 hover:scale-105 hover:shadow-primary/50 active:scale-95 transition-all group"
                    >
                      <svg className="w-6 h-6 flex-shrink-0 transition-transform group-hover:translate-x-1" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                  ...
                  {error && (
                    <div className="flex items-center justify-center gap-2 mt-4 py-2.5 bg-red-500/10 dark:bg-red-500/20 rounded-xl animate-fade-in border border-red-500/20">
                       <span className="w-1.5 h-1.5 rounded-full bg-red-500 animate-pulse"></span>
                       <p className="text-red-700 dark:text-red-400 text-[10px] font-black uppercase tracking-tight">{error}</p>
                    </div>
                  )}
                </div>
              </form>
              ...
              <div className="mt-10 text-center">
                 <p className="text-[9px] font-black text-slate-500 dark:text-slate-400 uppercase tracking-[0.4em] opacity-40">PROTECTED BY 4RISE</p>
              </div>
           </div>
        </div>
      )}
