...
const MediaItem: React.FC<{ item: PortfolioItem; isMinimal?: boolean; useZoomLoop?: boolean; isModal?: boolean }> = ({ item, isMinimal, useZoomLoop, isModal }) => {
  ...
  if (type === '3d' || type === 'certificate' || type === 'image' || type === 'movie' || type === 'video' || (type === 'web' && isUrlImage(url))) {
    return (
                      ...
                      <div className="columns-1 md:columns-2 lg:columns-3 xl:columns-4 gap-4 md:gap-6 space-y-4 md:space-y-6">
                         {sectionItems.map((item, idx) => (
                            <div 
                              ...
                              className={`group relative break-inside-avoid block w-full rounded-[1.2rem] md:rounded-[2rem] border border-slate-200 dark:border-slate-800 overflow-hidden shadow-lg hover:shadow-2xl hover:border-primary/50 dark:hover:border-primary/50 cursor-pointer transition-all duration-300 hover:-translate-y-1 opacity-0 animate-scale-up animation-delay-${Math.min(1000, 300 + (idx * 50))}`}
                            >
                               <MediaItem item={item} />
                               ...
                               {isCreator && (
                                 <button 
                                    ...
                                    className={`absolute top-3 right-3 p-2 rounded-full backdrop-blur-xl border transition-all duration-300 active:scale-75 ${item.isFeatured ? 'bg-rose-500/30 border-rose-500/50 text-rose-500 shadow-md' : 'bg-white/10 border-white/20 text-white/70 hover:bg-white/30 hover:text-white'}`}
                                 >
                                    <svg xmlns="http://www.w3.org/2000/svg" className="h-4 w-4" fill={item.isFeatured ? "currentColor" : "none"} viewBox="0 0 24 24" stroke="currentColor">
                                      <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2.5} d="M4.318 6.318a4.5 4.5 0 000 6.364L12 20.364l7.682-7.682a4.5 4.5 0 00-6.364-6.364L12 7.636l-1.318-1.318a4.5 4.5 0 00-6.364 0z" />
                                    </svg>
                                 </button>
                               )}

                               <div className="absolute bottom-0 left-0 w-full text-white z-10 p-4 md:p-5">
                                  <h3 className="font-bold text-sm md:text-base text-white group-hover:text-primary transition-colors line-clamp-2 leading-tight drop-shadow-lg">
                                     {item.title}
                                  </h3>
                                  {item.tags.length > 0 && (
                                     <div className="flex flex-wrap gap-1.5 mt-3 opacity-0 group-hover:opacity-100 transition-opacity duration-300">
                                       {item.tags.slice(0, 3).map(tag => (
                                         <span key={tag} className="px-2 py-0.5 bg-white/10 backdrop-blur-md rounded-md text-[9px] font-bold text-white/80">#{tag}</span>
                                       ))}
                                     </div>
                                  )}
                               </div>
      ...
      {selectedItem && (
        ...
        >
           ...
           {selectedItem.mediaType === 'certificate' ? (
              // SIDE-BY-SIDE LAYOUT FOR CERTIFICATES
              <div
                className="relative w-full max-w-5xl max-h-[90vh] bg-white dark:bg-card rounded-2xl md:rounded-3xl overflow-hidden shadow-2xl border border-slate-200 dark:border-slate-700 animate-scale-up flex flex-col md:flex-row"
                onClick={(e) => e.stopPropagation()}
              >
                <button 
                  onClick={() => handleSelectItem(null)} 
                  className="absolute top-4 right-4 z-50 p-3 bg-slate-100/50 dark:bg-slate-900/50 backdrop-blur-xl border border-white/20 dark:border-slate-700/50 text-slate-800 dark:text-white hover:bg-white/80 dark:hover:bg-slate-800 rounded-full transition-all group active:scale-90 shadow-lg"
                >
                  <svg className="w-5 h-5 transition-transform group-hover:rotate-90" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={3} d="M6 18L18 6M6 6l12 12" /></svg>
                </button>

                <div className="w-full md:w-3/5 bg-slate-100 dark:bg-slate-900/50 p-6 flex items-center justify-center overflow-hidden aspect-[4/3] md:aspect-auto">
                  <img src={selectedItem.mediaUrl} alt={selectedItem.title} className="max-w-full max-h-full object-contain rounded-lg shadow-lg"/>
                </div>

                <div className="w-full md:w-2/5 p-6 md:p-10 flex flex-col justify-between overflow-y-auto">
                  <div>
                    <div className="flex flex-wrap items-center gap-2.5 mb-4">
                      <span className="px-4 py-1.5 bg-primary/10 text-primary text-[9px] font-black uppercase tracking-widest rounded-full border border-primary/20">{selectedItem.mediaType}</span>
                      {selectedItem.isFeatured && (
                        <div className="flex items-center gap-2 px-3 py-1.5 bg-rose-500/10 text-rose-500 text-[9px] font-black uppercase tracking-widest rounded-full border border-rose-500/20">
                    ...
                    <h2 className="font-black text-2xl md:text-3xl text-slate-900 dark:text-white leading-tight tracking-tight mb-3">{selectedItem.title}</h2>
                    <div className="flex flex-wrap gap-2 mb-6">
                      {selectedItem.tags.map(tag => (
                        <span key={tag} className="px-3 py-1 bg-slate-100 dark:bg-slate-800 rounded-md text-[9px] font-bold text-slate-500 dark:text-slate-400 uppercase tracking-widest border border-slate-200 dark:border-slate-700/50">#{tag}</span>
                      ))}
                    </div>
                    <p className="text-sm text-slate-600 dark:text-slate-300 leading-relaxed whitespace-pre-line">{selectedItem.description}</p>
                  </div>
                  ...
                  <div className="mt-8 pt-6 border-t border-slate-200 dark:border-slate-700 flex flex-col sm:flex-row gap-4">
                     {selectedItem.projectUrl && (
                        <a href={selectedItem.projectUrl} target="_blank" rel="noopener noreferrer" className="w-full flex items-center justify-center gap-2.5 px-6 py-3 bg-primary text-white font-bold text-xs uppercase tracking-widest rounded-xl shadow-lg shadow-primary/20 hover:bg-primary/90 transition-all active:scale-95">
                          LIHAT VERIFIKASI
                      ...
                      {isCreator && (
                        <div className="flex gap-2.5 w-full">
                           <button onClick={() => { handleToggleFeatured(selectedItem.id); handleSelectItem(getPortfolioData().find(i => i.id === selectedItem.id) || null); }} className={`flex-1 p-3 rounded-xl border transition-all shadow-md ${selectedItem.isFeatured ? 'bg-rose-500/20 border-rose-500/50 text-rose-500' : 'bg-slate-100 dark:bg-slate-800 border-slate-200 dark:border-slate-700 text-slate-500 hover:bg-slate-200 dark:hover:bg-slate-700'}`}><svg xmlns="http://www.w3.org/2000/svg" className="h-5 w-5 mx-auto" fill={selectedItem.isFeatured ? "currentColor" : "none"} viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2.5} d="M4.318 6.318a4.5 4.5 0 000 6.364L12 20.364l7.682-7.682a4.5 4.5 0 00-6.364-6.364L12 7.636l-1.318-1.318a4.5 4.5 0 00-6.364 0z" /></svg></button>
                           <button onClick={() => { handleDelete(selectedItem.id); handleSelectItem(null); }} className="flex-1 p-3 bg-red-500/10 border border-red-500/30 text-red-500 rounded-xl hover:bg-red-500 hover:text-white transition-all shadow-md"><svg className="w-5 h-5 mx-auto" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M19 7l-.867 12.142A2 2 0 0116.138 21H7.862a2 2 0 01-1.995-1.858L5 7m5 4v6m4-6v6m1-10V4a1 1 0 00-1-1h-4a1 1 0 00-1 1v3M4 7h16" /></svg></button>
                        </div>
                      )}
                  </div>
                </div>
              ...
              // IMMERSIVE LAYOUT FOR ALL OTHER TYPES
              <div 
