...
const FloatingChat: React.FC<FloatingChatProps> = ({ isOpen, onToggle, onClose }) => {
  ...
  return (
    <div className="fixed bottom-24 md:bottom-10 right-6 z-[200] flex flex-col items-end gap-4 pointer-events-none">
      {/* Chat Window */}
      <div 
        className={`w-72 md:w-80 max-w-[calc(100vw-3rem)] transition-all duration-500 transform origin-bottom-right shadow-[0_30px_100px_rgba(0,0,0,0.4)] rounded-[2rem] overflow-hidden ${
          isOpen 
      ...
      >
        <div className="bg-white/95 dark:bg-slate-900/95 backdrop-blur-2xl border border-white/40 dark:border-slate-800/60">
      ...
      <button 
        onClick={onToggle} 
        className={`hidden md:flex pointer-events-auto relative group w-16 h-16 rounded-full items-center justify-center transition-all duration-500 shadow-2xl ${
          isOpen 
            ? 'bg-slate-900 dark:bg-white text-white dark:text-slate-900 rotate-[135deg] scale-90' 
